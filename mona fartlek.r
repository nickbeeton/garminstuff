...
start = 327 # time in seconds at which you start your Mona Fartlek

# extraction
coords = unlist(lapply(strsplit(X[grep('<trkpt', X)], '"'), function(x) x[c(2,4)]))
coords = t(matrix(as.numeric(coords), 2, length(coords)/2))
coords = coords[,2:1]
coords.mga55 = project(coords, '+proj=utm +zone=55 +south +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs', inv = FALSE) # set for eastern Australia
...
par(cex=1.5, lwd=2, bg = '#000000', col.axis = 'white', col.lab = 'white', col.main='white')
plot(time[-1]/60, vel, type='o', 
     ylim = 60/rev(c(2.75, 6.25)), pch=19, cex=0.5, xlab = 'Time (mins)', 
     ylab = 'Pace (min/km)', axes = FALSE, xlim = start/60 + c(0,20), col = '#656565')
axis(1, col = 'white')
axis(2, at = 60/seq(3,6,0.5), labels = c('3:00', '3:30', '4:00', '4:30', '5:00', '5:30', '6:00'), las = 1, col='white')
box(bty = 'O', col='white')

laps = start + c(90*(0:4), 360+60*(1:8), 840+30*(1:8), 1080+15*(1:8))
abline(v = c(0,laps)/60, col='grey', lty=2)
abline(h = 60/c(seq(3,4.5,0.25), seq(5, 6.5, 0.5)), col='grey', lty=2)
t.dist = cumsum(dist)
